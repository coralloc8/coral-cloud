package com.example.spring.common.cache;
...
import com.example.spring.common.ThreadPool;
import com.example.spring.common.exception.CacheSaveException;
...
public class LocalCacheServiceImpl<T> implements ICacheService<String, T> {
    ...
    public void init() {
        RemovalListener<String, T> removalListener =
            ...
            }, ThreadPool.INSTANCE.getThreadPool("com.example.spring.common.cache-pool", 1, 5));
...
//             )
    ...
    public Optional<T> getIfPresent(String key, Class<?> clazz) {
        try {
            ...
            log.error("#####com.example.spring.common.cache get error", e);
        }
    ...
    public T get(String key, Supplier<T> supplier, Class<?> clazz) {
        try {
            ...
            throw new CacheSaveException("failed to set com.example.spring.common.cache");
        }
