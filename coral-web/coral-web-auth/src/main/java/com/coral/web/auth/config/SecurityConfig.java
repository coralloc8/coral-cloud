...
import com.coral.database.test.entity.SysUser;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    /**
     * 认证服务器不查询用户的权限资源，只负责认证 注册一个UserDetailsService用于用户身份认证
     *
     * @param oauth2Service
     * @return
     */
    ...
    public UserDetailsService userDetailsService(IOauth2Service oauth2Service) {
        return username -> {
            ...
            List<String> roles = oauth2Service.findRolesByUser(user.getNo());

            // 权限暂时先忽略
            return User.withUsername(username).password(passwordAfterEncoder).roles(roles.toArray(new String[] {}))
                .build();
        };
    }
